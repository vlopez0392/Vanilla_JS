//Mini Prokect Credit Card Checker
...
function lunh(array){
  arrLen = array.length;
  const startIdx = arrLen - 1; //Furthermost digit
  let checkArray = [];
  ...
  for(let i = startIdx; i >= 0 ; i-- ){
    ...
    if(arrLen%2===0){
      i%2===0?checkArray.unshift(doubleElement(array[i])):checkArray.unshift(array[i]);
    }else{
      i%2!==0?checkArray.unshift(doubleElement(array[i])):checkArray.unshift(array[i]);
    }
  }

  let checkSum = checkArray.reduce((acc,curr)=>{
    return acc+curr;
  });
...
function findInvalidCards(nestedArray){
 ...
 console.log(`${findInvalidCards(batch).length} credit cards were found to be invalid!`);
...
function idInvalidCardCompanies(nestedArray){
  let invalidCards = findInvalidCards(nestedArray), firstDigits = [], 
  mailingCompanies =[];
  const ccCompanies = [['Amex',3], ['Visa',4],['MasterCard',5], ['Discover',6]];
  
  invalidCards.forEach(ccNumber => {
    firstDigits.push(ccNumber[0]);
  });

  for(let i = 0; i<ccCompanies.length; i++){
    if(firstDigits.indexOf(ccCompanies[i][1])>=0){
      mailingCompanies.push(ccCompanies[i][0]);
    }
  }
